...
// TestCreateStreamSstp_ConfigFileBody proves the --bootstrap-file input mode reads
...
// --bootstrap-file is a clear error before any request is made.
func TestCreateStreamSstp_RejectsFlagsWithConfig(t *testing.T) {
	...
	cmd := &CreateStreamSstpCmd{
		ClientAlias:   "clientNode",
		ServerAlias:   "serverNode",
		BootstrapFile: "/tmp/asym.json",
		Iss:           "tx.example.com",
	}

	err := cmd.Run(cli)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "bootstrap-file")
}
