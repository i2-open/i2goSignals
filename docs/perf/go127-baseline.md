...
| #278 — ML-DSA-65 SET signing (RFC 9964) | **-1.4%** `BenchmarkGenerateJti` (A/B) — no row regressed | **+1.0%** (54.07s vs 53.52s) | **No benchmark got slower.** This slice's numbers are an A/B against the branch's own HEAD (`6ea0184`) with the benchmark files held constant, for the reason [#279](https://github.com/i2-open/i2goSignals/issues/279) gives in the row above — the reference absolutes have drifted on this machine — and run with `LOG_LEVEL=ERROR` so the poll buffer's WARN does not interleave with the benchmark output. Every shared benchmark improved: the *worst* delta in the whole set is **-1.4%**, and the range runs to -10.5% (`BenchmarkNormalizePayload/v2/map-with-struct`). That is the expected shape — the only pre-existing code this slice changes on a benchmarked path is `AllowedAlgs()`, which grows from a two- to a three-element string slice consulted once per parse ahead of a signature verification, and `BenchmarkSetParse` came in at **-3.9%** with allocations flat at 77. **The three new rows are the actual result.** `BenchmarkSetJWSMLDSA` (`pkg/goSet`, new): **352,367 ns/op, 36 allocs/op, 26,392 B/op** against `BenchmarkSetJWS`'s 827,215 / 37 / 7,235 in the same run — signing a SET with ML-DSA-65 is **2.3× faster** than with RSA-2048, because an RSA private-key operation is expensive and a lattice signature is not. `BenchmarkSetParseMLDSA` (new): **99,270 ns/op, 66 allocs/op** against `BenchmarkSetParse`'s 31,099 / 77 — a verified parse is **3.2× slower**, because RSA verification with `e=65537` is close to free by construction. So the transmitter gets cheaper and the receiver gets dearer; neither is large against a network hop, and both are per-stream. **`BenchmarkSetWireSizeByAlg` (new) is the number that actually decides an opt-in** and reports no meaningful time — its custom `bytes/token` metric measures the wire cost on the benchmark set's representative SCIM SET: **1,113 bytes on RS256 → 5,188 on ML-DSA-65 (4.7×)**. A FIPS 204 ML-DSA-65 signature is 3,309 bytes where RSA-2048's is 256, and base64 turns that into ~4.1 KB of extra token. This is the whole reason `signing_alg` is per-stream and not a server posture ([ADR 0034](../adr/0034-ml-dsa-set-signing.md)); an operator turning it on should expect the poll `maxEvents` that was comfortable before to move considerably more bytes. Suite wall-clock is **+1.0%**, which is this slice's own tests rather than its code: `cmd/genTlsKeys` gains an ML-DSA CA + leaf issuance and a real TLS 1.3 handshake (~1s → ~3.5s), and the new key-service and round-trip suites each mint ML-DSA key pairs. Goldens moved by exactly one member — `signing_alg` appears in the two *populated* fixtures and nowhere else, with the zero and mixed fixtures byte-identical, which is the `omitzero` guarantee holding. |
