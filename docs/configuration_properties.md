...
| `I2SIG_PUSH_CONCURRENCY`              | RFC 8935 POSTs a push stream keeps in flight at once (worker pool per stream). Unset, it is derived from the processors the server can see (ADR 0037). The loop drains up to 4x this many buffered JTIs per batch, reads and acks them as a batch, and stops dispatching on the first failure; delivery order inside a batch is not guaranteed. **That 4x is also the ack-deferral window** — a crash mid-batch resends up to that many SETs the receiver already accepted (32 at the floor, 56 on a 14-processor host, 128 at the ceiling), which receivers drop on `jti` dedup. **Raising it trades ingest throughput for delivery throughput**: delivery and ingest compete for the same processors, and on the ADR 0037 sweep going from 5 to 24 took push from 417 to 583 ev/s while ingest fell from 1021 to 619 ev/s. Pin it low to favour ingest admission, high to feed a slow receiver, `1` for serial POSTs. An explicit value is not clamped. See ADR 0035 and ADR 0037. | `GOMAXPROCS`, clamped to 8..32 |
