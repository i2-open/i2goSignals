...
| `LOG_FORMAT`                 | Log output format. `text` is the default human-readable key=value form; `json` emits one parseable JSON object per line for collectors like Grafana Alloy, Fluent Bit, or the CloudWatch agent. See `docs/observability.md`. | `text`                                                 |
...
| `I2SIG_CLUSTER_NAME`         | Logical cluster identifier emitted as the `cluster_name` attribute on every log record. Purely observability metadata — does not affect lease semantics or cluster behavior. Omitted from log output when empty. | _none_                                                 |
...
| <BR>**Poll Transmitter — Long-Poll Timeouts** |                                                                                                                          |                                                        |
| `POLL_DEFAULT_TIMEOUT`       | Integer seconds. Long-poll timeout applied to inbound poll requests when the receiver omits `timeoutSecs` (or sends `0`). Set to `0` to disable implicit long-polling — empty buffer + omitted `timeoutSecs` returns immediately. Read once at startup; must be set uniformly across cluster nodes to avoid receiver-visible variance.              | `30`                                                   |
| `POLL_MAX_TIMEOUT`           | Integer seconds. Cap applied to inbound receiver-supplied `timeoutSecs`. Values above this are silently clamped (RFC8936 §2.4 makes `timeoutSecs` a SHOULD, so clamping is spec-compliant). Set to `0` to disable the cap entirely — restores the pre-cap behaviour and is the documented opt-out for operators that need unbounded long-polls. Read once at startup; must be set uniformly across cluster nodes. | `300`                                                  |
| <BR>**Push Transmitter — Recovery & Keepalive** |                                                                                                                          |                                                        |
...
| `I2SIG_PUSH_RETRY_LIMIT`             | Total elapsed wall time inside TransportBackoff before the stream is disabled. The longest-tail recovery cap.               | `6h`                                                   |
| `I2SIG_PUSH_UNAUTHORIZED_RETRY_DELAY`| Sleep between `/status` probes while in AuthBounded recovery (HTTP 401 path).                                              | `15s`                                                  |
| `I2SIG_PUSH_UNAUTHORIZED_RETRY_LIMIT`| Maximum AuthBounded probe attempts before the stream is disabled.                                                          | `10`                                                   |
| `I2SIG_PUSH_STATUS_CHECK_INTERVAL`   | Cadence at which recoveryLoop re-checks `/status` once it has confirmed the receiver is paused (PausedByRemote mode).       | `30s`                                                  |
| `I2SIG_PUSH_IDLE_VERIFY_INTERVAL`    | Idle period after which the push loop generates a real SSF verification SET as a keepalive. Set to `0` to disable T3.       | `5m`                                                   |
| <BR>**TLS Config**           |                                                                                                                                            |                                                        |
