...
- **Read in `pkg/ssfModels`, not injected.** The catalog is consumed by the CLI,
  ...
  cannot disagree with itself. `pkg/ssfModels` stays free of `internal/` imports;
  the deprecated-alias shim (`internal/envcompat`) is not needed because the
  variable is new and has no pre-v0.11.0 spelling.
...
- **A `PATCH` re-negotiates against the live catalog.** `UpdateStream` previously
  fell back to the stream's *stored* `events_supported` when the request omitted
  it. That snapshot is taken at create, so re-negotiating against it would make a
  newly configured vocabulary permanently unreachable for every pre-existing
  stream — delete-and-recreate as the only remedy. The stored catalog is
  refreshed alongside, keeping `events_delivered ⊆ events_supported`.
...
- Extension is registration-time, not retroactive: records already persisted keep
  the `events_delivered` they negotiated until they are updated or recreated.

## Related

- ADR 0011 — `I2SIG_<AREA>_*` environment-variable taxonomy (the `EVENT` area).
