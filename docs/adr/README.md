...
| [0031](0031-sstp-inbound-route-mode.md) | SSTP inbound honours its direction's `RouteMode` | `HandleEvent`'s unconditional `if sstpInbound { return nil }` guard treated every SSTP-ingested SET as `IMPORT`, silently dropping hop 2 of `A --sstp--> goSignals --sstp--> B`. The guard now reads the inbound direction's `RouteMode`: `IMPORT` stays terminal, `FORWARD`/`PUBLISH` fan out exactly as an RFC 8935 push receiver does. The forward-vs-re-sign choice stays at delivery time on the **outbound** stream. `IsReceiver()` cannot carry the check (SSTP is `ReceiveSstp`, not `ReceivePush`/`ReceivePoll`), and a blank inbound `RouteMode` is read as `IMPORT` so legacy records do not start fanning out on upgrade. `routeEventToSstpPairsLocked` takes an `excludePairId` so a SET is never echoed back out the pair it arrived on. |
