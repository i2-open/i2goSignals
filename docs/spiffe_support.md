...
| `I2SIG_STORE_MONGO_SPIFFE_ENABLED` | Enable SPIFFE mTLS for MongoDB connections | `false` |
...
When `I2SIG_STORE_MONGO_SPIFFE_ENABLED=true` and `SPIFFE_ENDPOINT_SOCKET` is set:
...
cluster-monitor:
  environment:
    ...
    - I2SIG_STORE_MONGO_SPIFFE_ENABLED=true
...
env:
  ...
  - name: I2SIG_STORE_MONGO_SPIFFE_ENABLED
    value: "true"
```
...
- **MongoDB credentials eliminated** — with `I2SIG_STORE_MONGO_SPIFFE_ENABLED=true` the MONGO_URL contains no username or password. Credential rotation is not required.
