...
type StreamConfiguration struct {
	...
	Aud []string `json:"aud,omitempty" bson:"aud,omitempty"`
	...
	EventsSupported []string `json:"events_supported,omitempty" bson:"events_supported,omitempty"`
	...
	EventsRequested []string `json:"events_requested" bson:"events_requested"`
	...
	EventsDelivered []string `json:"events_delivered,omitempty" bson:"events_delivered,omitempty"`
	...
	ReceiverJWKSUrl string `json:"receiverJWKSUrl,omitempty" bson:"receiverJWKSUrl,omitempty"`

	// Read-Only. Url for the event issuer public key
	IssuerJWKSUrl string `json:"issuerJWKSUrl,omitempty" bson:"issuerJWKSUrl,omitempty"`

	// Used to reset a stream to a specific date, if available events will be loaded >= date
	ResetDate *time.Time `json:"resetDate,omitempty" bson:"resetDate,omitempty"`
	// Used to reset a stream to a specific jti (assuming jti's are sortable). Events equal to or since the JTI will be available
	ResetJti string `json:"resetJti,omitempty" bson:"resetJti,omitempty"`
	...
	TxToken *string `json:"tx_token,omitempty" bson:"tx_token,omitempty"`

	// TxAlias is the alias of a model.Server which holds a security credential enabling remote management via SSF
	TxAlias *string `json:"tx_alias,omitempty" bson:"tx_alias,omitempty"`
...
func (sc *StreamConfiguration) DeepCopy() StreamConfiguration {
	...
	if sc.TxAlias != nil {
		copyAlias := *sc.TxAlias
		res.TxAlias = &copyAlias
	}
	if sc.RemoteStreamId != nil {
		copyId := *sc.RemoteStreamId
		res.RemoteStreamId = &copyId
	}
	return res
}
