...
type UpdateStreamStatus struct {
	// REQUIRED by SSF §8.1.2.2 in the request body. Identifies the stream whose
	// status is being updated. The SUT also accepts identification via the bearer
	// token (authCtx.StreamId), so this is omitempty for backward compatibility,
	// but spec-conformant receivers MUST send it.
