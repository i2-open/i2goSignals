...
func (suite *VerifySuite) TestTriggerVerification() {
	...
	streamToken, err := instance.GetAuthIssuer().IssueStreamToken(streamId, instance.projectId, nil)
	assert.NoError(t, err)
	...
	jtis, _ := instance.GetEventIds(streamId, model.PollParameters{MaxEvents: 10})
	assert.Len(t, jtis, 1)

	// Get the event and check its content
	event := instance.GetEvent(jtis[0])
