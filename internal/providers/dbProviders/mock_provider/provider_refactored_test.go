...
func TestMockProviderV2EventOperations(t *testing.T) {
	...
	eventRecord := provider.AddEvent(&event, "stream-1", `{"jti":"`+testJti+`"}`)
	if eventRecord == nil {
...
func TestMockProviderV2KeyOperations(t *testing.T) {
	...
	// Create a key pair
	privateKey := provider.CreateIssuerJwkKeyPair("test-issuer", "test-project")
	if privateKey == nil {
