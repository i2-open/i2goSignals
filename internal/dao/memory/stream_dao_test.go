...
func TestStreamDAOMemory_FindReceiverStreams(t *testing.T) {
	...
	// Create receiver stream (RouteMode = import)
	receiverStream := &model.StreamStateRecord{
		Id:        bson.NewObjectID(),
		ProjectId: "test-project",
		StreamConfiguration: model.StreamConfiguration{
			Id:        "receiver-stream",
	...
	_ = dao.Create(ctx, transmitterStream)

	// Find receiver streams
	receivers, err := dao.FindReceiverStreams(ctx)
	if err != nil {
		t.Fatalf("FindReceiverStreams failed: %v", err)
	}

	if len(receivers) != 1 {
		t.Errorf("Expected 1 receiver stream, got %d", len(receivers))
	}

	if receivers[0].StreamConfiguration.Id != "receiver-stream" {
		t.Errorf("Expected receiver-stream, got %s", receivers[0].StreamConfiguration.Id)
	}
}
